...
int controller_loadFromText(char* path, LinkedList* dominio)
{
    ...
    if(path!=NULL && dominio!=NULL)
    {
        pDominio=fopen(path,"r");
        if(!parser_dominioFromText(pDominio,dominio))
        {
            retorno=0;
        }
    }
...
/*int controller_loadFromBinary(char* path, LinkedList* dominio)
{
    ...
    if(path!=NULL && dominio!=NULL)
    {
        pDominio = fopen(path,"rb");
        if(!parser_dominioFromBinary(pDominio,dominio))
        {
            retorno = 0;
        }
    }
...
{
    int retorno =-1;
    eDominio* auxiliarPuntero =NULL;

    char bufferId[128];
    char bufferDominio[128];
    char bufferAnio[128];

    printf("A L T A   DOMINIO.");
    if(dominio!=NULL)
    {

        if( !funciones_getStringInput("INGRESE id dominio: ",128,bufferId) &&
                !funciones_getStringInput("INGRESE dominio: ",128,bufferDominio) &&
                !funciones_getStringInput("INGRESE anio: ",128,bufferAnio))
        {
            if( funciones_isID(atoi(bufferId)) == 1)

            {
                auxiliarPuntero = (eDominio*)dominio_newParametros(bufferId, bufferDominio, bufferAnio);
            }
            else
            {
        ...
        if(auxiliarPuntero != NULL)
        {
            if(!ll_add(dominio,auxiliarPuntero))
            {
                retorno = 0;
            }
...
int controller_ListDominio(LinkedList* dominio)
{
    ...
    if(dominio !=NULL)
    {
        eDominio* auxiliarPuntero;

        auxiliarPuntero= dominio_new();

        lenList = ll_len(dominio);
        printf("ID:  - dominio:               - anio:");
        for(i=0; i<lenList; i++)
        {
                printf("\n");

                auxiliarPuntero = (eDominio*)ll_get(dominio,i);


                if(auxiliarPuntero != NULL)
                {
                    dominio_printDominio(auxiliarPuntero);
                    retorno = 0;
                }
            }
...
int controller_menuDeOpciones(void)
{
    ...
    printf("O  1)ABRIR UN ARCHIVO.\n");
    printf("8                                                                 8\n");
    printf("O  2)Listar DOMINIOS.\n");
    printf("8                                                                 8\n");
    printf("OoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoOoO\n");
