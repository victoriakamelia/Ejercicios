 ...
 *
 */
int parser_dominioFromText(FILE* pFile, LinkedList* dominio)
{
    ...
    int contador=0;
    char bufferId[10];
    char bufferDominio[10];
    char bufferAnio[10];
    //char bufferTipo[10];


    if(pFile!= NULL && dominio!=NULL)
    {
        ...
        do
        {
            ...
            else
            {
                          ...
                          bufferAnio) == 3)
                {
                    ...
                    {
                        ll_add(dominio, auxiliarPuntero);
                        contador++;
                    }
                    else
                    {
                        printf("\nNO FUE POSIBLE AGREGAR AL dominio ID %s\n", bufferId);
                    }
                }
                else
                {
        ...
        while(!feof(pFile));
        fclose(pFile);
        printf("\n SE CARGARON DE FORMA EXITOSA %i DE dominio\n",contador);
        retorno = 0;
    }
